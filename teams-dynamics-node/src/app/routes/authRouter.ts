...
export const authRouter = (options: any): express.Router => {
    ...
    const templateAuthzUrl = "https://login.windows.net/" + process.env.TENANT_NAME + ".onmicrosoft.com/oauth2/authorize?response_type=code&client_id=<client_id>&redirect_uri=<redirect_uri>&state=<state>&resource=<resource>";
    const redirectUri = `https://${process.env.hostname}/api/auth/getAToken`;
    const resource = `https://${process.env.TENANT_NAME}.crm4.dynamics.com`;
    const createAuthorizationUrl = (state: string) => {
        let authorizationUrl = templateAuthzUrl.replace("<client_id>", process.env.CLIENT_APP_ID as string);
